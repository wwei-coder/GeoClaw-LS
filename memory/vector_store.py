...
from memory.rerank import get_reranker
...
class VectorStore:
    ...
    def _build_bm25_index(self, force_rebuild: bool = False):
        ...
        try:
            ...
            # Fetch all documents (warning: heavy for large datasets)
    ...
    def add_chunks(self, new_chunks: List[Dict[str, Any]], refresh_index: bool = True):
        ...
        for c in new_chunks:
             meta = {
                 ...
                 "parent_content": c.get("parent_content", ""), # Store parent content for retrieval
                 "active": True
             }
             year = c.get("year")
    ...
    def _search_vector(self, query: str, top_k: int, filter: Optional[Dict[str, Any]] = None) -> List[Dict[str, Any]]:
        ...
        for i in range(len(results["ids"][0])):
             ...
             chunk = {
                 "id": doc_id,
                 "content": doc_text,
                 "metadata": meta, # Keep full metadata
                 "doc_name": meta.get("doc_name", ""),
                 "parent_content": meta.get("parent_content", ""),
                 "distance": dist,
                 "score": 1.0 / (1.0 + dist) # Convert distance to score-like
             }
    ...
    def _search_bm25(self, query: str, top_k: int, filter: Optional[Dict[str, Any]] = None) -> List[Dict[str, Any]]:
        ...
        # Get top n
        top_n_indices = sorted(range(len(doc_scores)), key=lambda i: doc_scores[i], reverse=True)[:top_k * VECTOR_SEARCH_CANDIDATE_MULTIPLIER]
        ...
        # include=['documents', 'metadatas'] is default for get? No, need to specify if we want specific
        # Default returns: metadatas, documents, embeddings (if included)
        fetched = self.collection.get(ids=ids_to_fetch, where=filter)
        
        # Map fetched data back to candidates
        fetched_map = {id: (doc, meta) for id, doc, meta in zip(fetched['ids'], fetched['documents'], fetched['metadatas'])}
        
        final_candidates = []
        for c in candidates:
            # Only include if it passed the filter (exists in fetched_map)
    ...
    def search(
        ...
        # 4. Rerank (if enabled)
        # Reranker expects list of dicts with 'content'
        if self.reranker:
        ...
        # 5. Format results and Apply Parent-Child Strategy
        final_results = []
        seen_docs = {}
        
        for res in fused_results:
             # Parent-Child: Use parent content if available
             # Note: 'parent_content' was added to metadata in add_chunks
             
             display_content = res.get("parent_content")
             if not display_content:
                 display_content = res["content"]
                 
             # Update the content to be returned to LLM
